...
@interface IITGoogleSpeechRecognizer () {;
    BOOL processing;
}
@property (nonatomic, readwrite) BOOL micPermission;
...
@implementation IITGoogleSpeechRecognizer
...
- (void)setMicPermission:(BOOL)micPermission
{
    if (_micPermission != micPermission) {
        [self willChangeValueForKey:@"micPermissionGranted"];
        _micPermission = micPermission;
        [self didChangeValueForKey:@"micPermissionGranted"];
    }
}
